...
export async function sendVerification(req, res) {
  ...
  await EmailVerification.findOneAndUpdate(
    ...
    { email: normalizedEmail, token, verified: false, expiresAt },
    { upsert: true, setDefaultsOnInsert: true },
  );

  const scheme = process.env.APP_SCHEME || 'myhealthbook';
  const verifyUrl = `${scheme}://verify?token=${token}&email=${encodeURIComponent(normalizedEmail)}`;
