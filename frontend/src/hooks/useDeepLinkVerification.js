...
/* Matches both the real https App Link (what the email button now
   uses) and the older bare custom-scheme link, just in case. */
const VERIFY_LINK_RE = /^(?:https:\/\/myhealthbook\.rehabiphy\.com|myhealthbook:\/\/)\/?verify\?(.+)$/i;
...
/* Dual-mode resolution: an instant deep-link callback if the app is
   still alive when the verify link is tapped, plus a 3s poll as a
   fallback for when the OS reclaimed the process in between.
   Whichever resolves first wins — onVerified is only ever meant to be
   called once, calling it twice is harmless. */
